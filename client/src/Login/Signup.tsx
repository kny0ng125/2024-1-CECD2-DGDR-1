...
const PILL_BUTTON_CLASS =
  'py-2.5 px-4 border-0 rounded-[5px] bg-dispatch-blue-soft text-[#93c5fd] text-[12px] font-semibold cursor-pointer ring-1 ring-inset ring-dispatch-blue-edge whitespace-nowrap'
      ...
      >
        ...
        <form onSubmit={handleSignup} className="flex flex-col gap-4">
          ...
          <div>
            ...
            {accountValid !== null && (
              <div className={`mt-1.5 text-[11px] ${accountValid ? 'text-[#4ade80]' : 'text-[#fca5a5]'}`}>
          ...
          <div>
            ...
            {phoneValid !== null && (
              <div className={`mt-1.5 text-[11px] ${phoneValid ? 'text-[#4ade80]' : 'text-[#fca5a5]'}`}>
          ...
          <div>
            ...
            <input type="password" placeholder="••••••••" value={password}
              ...
              className={INPUT_FULL_CLASS} />
            {password && (
              <div className={`mt-1.5 text-[11px] ${passwordValid ? 'text-[#4ade80]' : 'text-[#fca5a5]'}`}>
                {passwordValid
                  ? '비밀번호가 유효합니다.'
        ...
        {/* Result messages */}
        {signupSuccess && (
          <div className="mt-4 py-3 px-3.5 bg-dispatch-green-soft ring-1 ring-inset ring-dispatch-green-edge rounded-[5px] text-[#4ade80] text-[12px] text-center">
            회원가입이 완료되었습니다. 로그인 페이지로 이동합니다.
          </div>
        )}
        {errorMessage && (
          <div className="mt-4 py-3 px-3.5 bg-dispatch-red-soft ring-1 ring-inset ring-dispatch-red-edge rounded-[5px] text-[#fca5a5] text-[12px] text-center">
            {errorMessage}
          </div>
        )}
